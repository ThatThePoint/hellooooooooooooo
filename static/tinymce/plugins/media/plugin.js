...
var media = (function () {
  ...
  var getVideoHtml = function (data, videoTemplateCallback) {
    if (videoTemplateCallback) {
      ...
      return '<video src="' + data.source1 + '" width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls">\n' + '<source src="' + data.source1 + '"' + (data.source1mime ? ' type="' + data.source1mime + '"' : '') + ' />\n' + (data.source2 ? '<source src="' + data.source2 + '"' + (data.source2mime ? ' type="' + data.source2mime + '"' : '') + ' />\n' : '') + '</video>';
    }
  };
  ...
  var dataToHtml = function (editor, dataIn) {
    ...
    videoSource = data.source1 || data.source2;
  ...
  var createPreviewIframeNode = function (editor, node) {
    ...
    previewNode.attr({
      ...
      frameborder: '0',
      src:videoSource,//这是增加的属性，videoSource是从第三步来的
      controls:'controls'
    });
  ...
  var placeHolderConverter = function (editor) {
    return function (nodes) {
      ...
      while (i--) {
        ...
        } else if(node.name === 'video' && $_6qt1nth3jkmcwpf2.hasLiveEmbeds(editor) && global$1.ceFalse){
          node.replace(createPreviewIframeNode(editor, node));
        }else {
          if (!isWithinEphoxEmbed(node)) {
